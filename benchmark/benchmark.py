...
from chipcompiler.tools import build_step_metrics  # noqa: E402
from chipcompiler.utility import csv_write, json_read  # noqa: E402

from .get_parameters import get_parameters  # noqa: E402
...
def get_tasks(benchmark_json: str, target_dir: str = "", batch_name: str = "", design: str = ""):
    ...
    for design_info in designs:
        ...
        if not value_is_ok:
            raise ValueError(
                f"Invalid design info for {design_info.get('id', '')}, missing required fields"
            )

        # Collect task parameters
        design_tasks.append(
            (
                f"{task_dir}/{design_info.get('id', '')}",
                benchmarks.get("pdk", ""),
                design_info,
            )
        )
...
def run_single_design(workspace_dir: str, pdk_name: str, design_info: dict):
...
def benchmark_statis(benchmark_dir: str):
    ...
    for root, dirs, _files in os.walk(benchmark_dir):
    ...
    csv_write(file_path=statis_csv, header=[header], data=results)


def benchmark_metrics(benchmark_json: str, target_dir: str = "", batch_name: str = ""):
    ...
    for workspace_dir, pdk_name, design_info in design_tasks:
        ...
        for workspace_step in engine_flow.workspace_steps:
            step = engine_flow.get_step(name=workspace_step.name, tool=workspace_step.tool)
            if engine_flow.check_state(
                name=workspace_step.name, tool=workspace_step.tool, state=StateEnum.Success
            ):
                metrics = build_step_metrics(workspace=workspace, step=workspace_step)
                if metrics is None:
                    step_result.append(step["state"])
                else:
                    for index, (key, value) in enumerate(metrics.data.items()):
                        if init_header:
