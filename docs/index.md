...
- **[CLI User Guide](../chipcompiler/docs/ecc-user-guide.en.md)** / **[中文用户指南](../chipcompiler/docs/ecc-user-guide.cn.md)** - All currently supported commands
  - Every command and option: `version`/`init`/`check`/`run`/`status`/`log`/`config`/`migrate`/`doctor`/`param`/`macro`/`doc`/`pdk`/`project`/`workspace`/`signoff`/`report`/`rpc`/`layout-image`
