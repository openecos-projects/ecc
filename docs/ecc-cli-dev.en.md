...
- **Project mode** (default): read `ecc.toml` → resolve RTL/PDK/parameters plus the `--preset` override → for a fresh or `--overwrite` target, **environment preflight** (`_preflight_environment` probes bundled ecc-tools plus the preset's Yosys/DreamPlace use; anything missing → `env_not_ready` fail-fast) → `create_workspace` under the resolved run target (manifest/virgin projects default to `<project>/<run-id>` or the workspace path declared in `project.json`; `runs/<run-id>` is the legacy layout that `ecc migrate` upgrades) → build the `EngineFlow` from the steps in `rtl2gds.get_flow_builders()` → run (progress rendering via `rendering/progress.py::run_flow_with_progress` on a TTY, plain `run_steps` otherwise). Existing workspaces reconcile and resume their persisted flow without preset preflight. `--overwrite` runs safety checks first (only genuine ECC run directories are deleted); `--preset` is not written back to `ecc.toml` and is mutually exclusive with `--workspace`. Sizer is deliberately omitted from this preflight and fails when its Timing optimization step executes.
