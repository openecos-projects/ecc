...
def _run_step_in_subprocess(
    workspace: Workspace, workspace_step: WorkspaceStep, regenerate_config: bool = True
) -> None:
    """
...
class EngineFlow:
    def __init__(self, workspace: Workspace):
        ...
        self.reuse_existing_configs = False
    ...
    def has_init(self):
        return (
            True
            if self.workspace is not None and len(self.workspace.flow.data.get("steps", [])) > 0
            else False
        )
    ...
    def check_state(self, name: str, tool: str, state: str | StateEnum):
        ...
        state_value = state.value if isinstance(state, StateEnum) else state
        if step is not None and step.get("state") == state_value:
            return True
    ...
    def check_step_result(self, workspace_step: WorkspaceStep):
        ...
        match workspace_step.name:
            ...
            case default:
                if (
                    os.path.exists(workspace_step.output.get("def", ""))
                    and os.path.exists(workspace_step.output.get("verilog", ""))
                    and os.path.exists(workspace_step.output.get("gds", ""))
                ):
                    success = True
        return success

    def create_step_workspaces(self, reuse_existing_configs: bool = False):
        """
        Create all step workspaces.

        Args:
            reuse_existing_configs: When ``True``, rebuild the ``WorkspaceStep``
                objects and ensure directories exist, but keep the existing step
                config files instead of regenerating them from tool templates.
        """
        self.reuse_existing_configs = reuse_existing_configs
        pre_step = None
        for step in self.workspace.flow.data.get("steps", []):
            if pre_step is None:
            ...
            eda_step = create_step(
                ...
                build_config=not reuse_existing_configs,
            )
            # save workspace step
    ...
    def run_step(self, workspace_step: WorkspaceStep | str, rerun: bool = False) -> StateEnum:
        ...
        p = Process(
            target=_run_step_in_subprocess,
            args=(self.workspace, workspace_step, not self.reuse_existing_configs),
        )
        p.start()
        step_log_file = workspace_step.log.get("file", "")
        logger.info(
            "[DISPATCH] %s pid=%s log=%s",
            step_tag,
            p.pid,
