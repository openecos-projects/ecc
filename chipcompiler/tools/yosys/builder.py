...
def generate_global_var_tcl(workspace: Workspace, step: WorkspaceStep) -> str:
    ...
    if filelist:
        ...
        rtl_source_section = (
            f"# RTL source files\nset rtl_file            [split {_tcl_quote(_abspath(rtl_file))}]"
        )
...
def build_step_config(workspace: Workspace, step: WorkspaceStep):
    ...
    - script/ subdirectory with prepared TCL scripts
      (yosys_synthesis.tcl, init_tech.tcl, abc-opt.script, aig library)
    - data/ subdirectory with generated files
      (global_var.tcl, tmp/ folder with generated temporary constr files)
    """
    import shutil
...
def build_environment(workspace: Workspace, step: WorkspaceStep):
    """
