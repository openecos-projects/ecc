...
class DreamPlace:
    ...
    def _run(self, legalize_only: bool) -> bool:
        ...
        try:
            ...
            log_path = self._log_path(legalize_only)
            with open(log_path, "a") as f:
                f.write("\n[ERROR] engine.run() raised exception:\n")
                traceback.print_exc(file=f)
            LOGGER.error("dreamplace engine.run() raised exception for %s", self.step.name, exc_info=True)
            return False
