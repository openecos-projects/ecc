    ...
    if os.path.exists(step.input["def"]):
        ...
        if os.path.exists(step.input["verilog"]):
            eda_inst.read_verilog(
                verilog=step.input["verilog"], top_module=workspace.design.top_module
            )
        else:
            return None
...
def get_eda_instance(
    workspace: Workspace, step: WorkspaceStep, instance: ECCToolsModule = None
) -> ECCToolsModule:
    """
    ...
    if instance is not None:
        ...
        # init ecc module
        eda_inst = create_db_engine(workspace=workspace, step=step)

    return eda_inst


def save_data(step: WorkspaceStep, module: ECCToolsModule) -> bool:
    """
    module is ecc module from db engine,
    eda instacnce may initialize data from this module if module has been set
    """
    ...
    module.feature_step(step=step.name, json_path=step.feature["step"])
...
def run_step(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    if not is_eda_exist():
    ...
    match step.name:
        case StepEnum.FLOORPLAN.value:
            state = run_floorplan(workspace=workspace, step=step, module=module)
        case StepEnum.NETLIST_OPT.value:
            state = run_net_opt(workspace=workspace, step=step, module=module)
        case StepEnum.PLACEMENT.value:
            state = run_placement(workspace=workspace, step=step, module=module)
        case StepEnum.CTS.value:
            state = run_cts(workspace=workspace, step=step, module=module)
        case StepEnum.TIMING_OPT_DRV.value:
            state = run_timing_opt_drv(workspace=workspace, step=step, module=module)
        case StepEnum.TIMING_OPT_HOLD.value:
            state = run_timing_opt_hold(workspace=workspace, step=step, module=module)
        case StepEnum.LEGALIZATION.value:
            state = run_legalization(workspace=workspace, step=step, module=module)
        case StepEnum.ROUTING.value:
            state = run_routing(workspace=workspace, step=step, module=module)
        case StepEnum.DRC.value:
            state = run_drc(workspace=workspace, step=step, module=module)
        case StepEnum.FILLER.value:
            state = run_filler(workspace=workspace, step=step, module=module)
...
def run_net_opt(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
    ...
    eda_inst = get_eda_instance(workspace=workspace, step=step, instance=module)
...
def run_placement(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
    run placement
    """
    reslut = False
...
def run_cts(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
    run CTS
    """
    reslut = False
    ...
    if eda_inst is not None:
        ...
        eda_inst.run_cts(
            config=step.config[f"{StepEnum.CTS.value}"], output=step.data[f"{StepEnum.CTS.value}"]
        )
...
def run_timing_opt_drv(
    workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None
) -> bool:
    """
...
def run_routing(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
    run routing
    """
    reslut = False
...
def run_drc(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
    run chip drc
    """
    reslut = False
    ...
    if eda_inst is not None:
        ...
        eda_inst.run_drc(
            config=step.config[f"{StepEnum.DRC.value}"], report_path=step.report["step"]
        )
...
def run_filler(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
...
def run_floorplan(workspace: Workspace, step: WorkspaceStep, module: ECCToolsModule = None) -> bool:
    """
    run floorplan
    """
    ...
    if eda_inst is not None:
        ...
        sub_flow.update_step(step_name="init floorplan", state=StateEnum.Success)
        ...
        sub_flow.update_step(step_name="create tracks", state=StateEnum.Success)
        ...
        for item in json_io_pins:
            ...
            eda_inst.add_pdn_io(net_name=net_name, direction=direction, is_power=is_power)
        ...
        sub_flow.update_step(step_name="place io pins", state=StateEnum.Success)
        ...
        sub_flow.update_step(step_name="tap cell", state=StateEnum.Success)
        ...
        sub_flow.update_step(step_name="PDN", state=StateEnum.Success)

        # set clock net
        clock_name = workspace.parameters.data.get("Clock", "")
        eda_inst.set_net(net_name=clock_name, net_type="CLOCK")
        sub_flow.update_step(step_name="set clock net", state=StateEnum.Success)
