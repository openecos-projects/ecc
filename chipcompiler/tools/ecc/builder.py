...
def build_step(
    ...
    step.config = {
        ...
        f"{StepEnum.NETLIST_OPT.value}": os.path.join(
            step.directory, "config", "no_default_config_fixfanout.json"
        ),
        ...
        f"{StepEnum.TIMING_OPT_SETUP.value}": os.path.join(
            step.directory, "config", "to_default_config_setup.json"
        ),
    ...
    # build input paths
    step.input = {"def": input_def, "verilog": input_verilog}
    ...
    # build log paths
    step.log = {"dir": f"{step.directory}/log", "file": f"{step.directory}/log/{step.name}.log"}
    ...
    step.subflow = {"path": f"{step.directory}/subflow.json", "steps": []}

    # build checklist paths and data
    step.checklist = {"path": f"{step.directory}/checklist.json", "checklist": []}
...
def build_checklist(workspace: Workspace, workspace_step: WorkspaceStep):
...
def build_step_space(step: WorkspaceStep) -> None:
    ...
    for _key, dir in step.data.items():
...
def build_step_config(workspace: Workspace, step: WorkspaceStep):
    """
    ...
    def _update_db():
        ...
        config["LayerSettings"]["routing_layer_1st"] = workspace.parameters.data.get(
            "Bottom layer", ""
        )
