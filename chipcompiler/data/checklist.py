...
class Checklist:
    ...
    def state_value(self, state: str | CheckState) -> str:
    ...
    def add(self, step: str, type: str, item: str, state: str, info: str = ""):
        # check if exist
        for check_item in self.data.get("checklist", []):
            if (
                check_item["step"] == step
                and check_item["type"] == type
    ...
    def update(self, step: str, type: str, item: str, state: str | CheckState, info: str = ""):
        # check if exist
        for check_item in self.data.get("checklist", []):
            if (
                check_item["step"] == step
        ...
        # add to checklist
        self.add(step=step, type=type, item=item, state=self.state_value(state), info=info)
