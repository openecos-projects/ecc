...
from .parameter import Parameters, load_parameter, save_parameter
...
def create_workspace(
    ...
    workspace.logger = create_logger(
        name=workspace.parameters.data["Design"], log_dir=f"{directory}/log"
    )
    ...
    if os.path.exists(input_filelist):
        try:
            ...
            workspace.design.input_filelist = copy_filelist_with_sources(
                input_filelist=input_filelist, workspace_dir=directory, logger=workspace.logger
            )
        except Exception as e:
            ...
            workspace.design.input_filelist = (
                f"{directory}/origin/{os.path.basename(input_filelist)}"
            )
...
def load_workspace(directory: str) -> Workspace:
    ...
    # create logger first (needed for copy operations)
    workspace.logger = create_logger(name=parameters.data["Design"], log_dir=f"{directory}/log")
