...
`ecc version` formats version metadata directly; it also has a hidden `--json` flag (a single object with a version-specific schema) reserved for the desktop app and kept out of `--help`. `ecc rpc serve` and `ecc layout-image` intentionally do not use record-renderer output modes.
