...
Engineering Snapshot schema 5 is the only GUI input contract. It embeds a
bounded `qorSnapshotExtension` from this V3 engine; schema 4 and earlier
snapshots are rejected and must be regenerated by rebuilding the workspace.
...
graph LR
    ...
    S --> D["ECOS Studio<br/>(renderer: 5-dim breakdown / gates / diagnoses)"]
...
| ECOS Studio | Project dashboard QoR card, 5-dimension breakdown, diagnosis list | Reads `qorSnapshotExtension` from Engineering Snapshot schema 5; missing or stale → NOT_RATED (§10.2) |
...
`GREEN ≥90` / `YELLOW ≥75` / `ORANGE ≥60` / `RED <60`; a failed gate → `FAIL` (score 0); unevaluated → `NOT_RATED` (score null). ECOS Studio renders this scalar status directly and does not apply a second pass/fail threshold.
...
- Studio **never re-scores**: scores, statuses, and gates come only from the schema 5 Engineering Snapshot `qorSnapshotExtension` (validated for `scoringEngine: "qor-v3"`).
- **Staleness detection**: a mixed or stale Engineering Snapshot marks its
  `qorSnapshotExtension` unavailable → **NOT_RATED** — prefer no score over a
  wrong score. Rerunning the invalidated steps restores it.
...
- Engineering Snapshot metrics are grouped by their explicit `stepId`; no
  legacy assessment offsets or threshold fields are consumed.
...
One of: the workspace has not been rebuilt for Snapshot schema 5; the
`qorSnapshotExtension` is unavailable; feasibility is NOT_VERIFIED (a gate's
stage did not run successfully) or UNKNOWN (the stage ran but its evidence is
missing or corrupt); or no dimension is evaluable. Rerun the affected steps to
close it out.
