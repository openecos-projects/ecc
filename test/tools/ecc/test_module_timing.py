...
def test_run_timing_splits_text_reports_and_structured_artifacts(tmp_path):
    ...
    assert sorted(path.name for path in report_dir.iterdir()) == sorted(
        ["gcd.sdf", *module.ecc.text_report_filenames]
    )
...
def test_run_timing_allows_missing_power_report(tmp_path):
    module = make_module()
    ...
    assert (report_dir / "notes.txt").read_text(encoding="utf-8") == "keep\n"
    assert not (report_dir / "power.rpt").exists()
    assert not (feature_dir / "power_summary.json").exists()
