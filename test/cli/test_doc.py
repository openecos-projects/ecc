...
def test_all_four_topics_resolve_in_both_languages():
    for topic in docs.GUIDE_STEMS:
        for lang in ("en", "cn"):
            text = docs.load_guide(topic, lang)
            assert text.startswith("# ")


def test_doc_config_plain_is_byte_identical_to_the_guide_file(capsys):
    rc = cli_main.run(["doc", "config", "--plain"])

    out = capsys.readouterr().out
    assert rc == 0
    assert out.encode("utf-8") == (guides_root() / "ecc-cli-config.en.md").read_bytes()
