...
def chipcompiler_server_bundle(
        name,
        visibility = None,
    ...
    kwargs = {
        ...
        "tools": [pyinstaller],
        "outs": ["server_bundle/chipcompiler"],
        ...
        "cmd": """
            ...
            $(location {pyinstaller}) $(location ecc.spec) \
                --clean \
                --noconfirm \
                --distpath "$(@D)/server_bundle" \
                --workpath "$(@D)/server_bundle_work"

            if [ ! -f "$@" ]; then
                echo "ERROR: expected output not found at $@" >&2
                find "$(@D)/server_bundle" -maxdepth 3 -type f | sort >&2
                exit 1
            fi
        """.format(
